...
names(pwr_data) <- c("Date","Time","Glb_Act","Glb_Reac","Volt","Glb_Int","Sub1","Sub2","Sub3")
...
names(pwr_sub) <- c("Date","Glb_Act","Glb_Reac","Volt","Glb_Int","Sub1","Sub2","Sub3")

# Subset data for the dates needed

pwr_sub <- pwr_sub[{pwr_sub$Date > "2007-02-1" & pwr_sub$Date < "2007-02-03"},]

# The data is ready. Create all 4 plots
...
plot(pwr_sub$Date, pwr_sub$Volt, xlab = "Datetime", ylab = "Voltage" , type = "n")
lines(pwr_sub$Date,pwr_sub$Volt)
...
plot(pwr_sub$Date, pwr_sub$Glb_Reac, xlab = "Datetime", ylab = "Global Reactive Power" , type = "n")
lines(pwr_sub$Date,pwr_sub$Glb_Reac)
